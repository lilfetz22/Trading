...
import pickle
import gym_mtsim
from gym_mtsim_forked.gym_mtsim.data import FOREX_DATA_PATH_PRODUCTION_TESTING, FOREX_DATA_PATH, MODEL_PATH
...
if __name__ == '__main__':
    ...
    instrument = 'EURUSD'
    server_IP = '127.0.0.1'
    server_port = 2648  # check port number
    seed = 2024
    # IG Account info
    # 744127
    # w8gmsxe
    ...
    sim_production = gym_mtsim.MtSimulator(
        ...
        stop_out_level=0.2,
        hedge=True,
        symbols_filename=FOREX_DATA_PATH_PRODUCTION_TESTING
    )
    ...
    if (connection == True):
        ...
        while(forever):
            ...
            if (actual_bar_info['date'] > date_value_last_bar) and (not acct_protection):
                ...
                data_added = fx_rl.get_latest_data(FOREX_DATA_PATH_PRODUCTION_TESTING, df_new, instrument=instrument)
                if data_added:
                    count_of_times_getting_new_data += 1
                    sim_production.load_symbols(FOREX_DATA_PATH_PRODUCTION_TESTING)
                    ...
                    if (len(closed_tickets) > 0):
                        # print(f'length of closed tickets: {len(closed_tickets)}')
